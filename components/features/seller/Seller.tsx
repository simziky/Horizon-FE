...
import LazyKeepaChart from "./LazyKeepaChart"
import { debounce } from "@/utils/debounce"
import FinalLoader from "@/components/features/dashboard/loader"
    ...
    <section className="flex flex-col gap-8 min-h-[50dvh] md:min-h-[80dvh] bg-white p-2 rounded-lg">
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* Product card skeletons while product list is loading */}
          {productLoading || loading ? (
            <div className="flex flex-col gap-6">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="grid sm:grid-cols-2 lg:grid-cols-[3fr_2fr_2fr] xl:grid-cols-[4fr_2fr_2fr] gap-2 gap-y-4 items-stretch animate-pulse">
                  {/* Product info skeleton — matches real card: image 166×197, icon row, title, rating, ASIN/UPC, category, price */}
                  <div className="rounded-xl border border-border p-3 flex flex-col divide-y divide-border">
                    <div className="flex flex-col sm:flex-row items-center gap-4 pb-3">
                      <div className="w-[166px] h-[197px] bg-gray-200 rounded-lg flex-shrink-0" />
                  ...
                  {/* Offers skeleton — matches "Top 5 Offers" table rows */}
                  ...
                  {/* Chart skeleton */}
                  <div className="rounded-xl border border-border bg-gray-200 min-h-[200px]" />
                </div>
              ))}
            </div>
          ) : (
                  ...
                  return (
                    <div key={index}>
                      <div className="grid sm:grid-cols-2 lg:grid-cols-[3fr_2fr_2fr] xl:grid-cols-[4fr_2fr_2fr] gap-2 gap-y-4 items-stretch">
                        ...
                        {/* Keepa Chart — mounts only when card enters viewport */}
