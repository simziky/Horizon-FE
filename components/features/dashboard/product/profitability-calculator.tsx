...
import { JSX, useState, useEffect, useCallback, useMemo, useRef, forwardRef, useImperativeHandle } from "react";
import { message, Skeleton, Tooltip as AntTooltip } from "antd";
import { debounce } from "lodash";
import { CustomSlider as Slider } from "@/components/ui/AntdComponents";
...
>(({
  ...
  const evaluateRef = useRef<((expression: string) => any) | null>(null);
  ...
  const buyboxDetails = useMemo(
    () => (buyboxDetailsData?.data?.buybox as BuyboxItem[]) ?? [],
    [buyboxDetailsData]
  );
  ...
  useEffect(() => {
    // Lazy-load mathjs when component mounts to keep it out of the initial chunk
  ...
  const evaluateExpression = (expression: string): { isValid: boolean; result: string } => {
    ...
    if (!trimmed) return { isValid: false, result: expression };

    // Fast path for simple numbers while mathjs is still loading
    const numeric = Number(trimmed);
    if (!Number.isNaN(numeric) && evaluateRef.current === null) {
      return { isValid: true, result: numeric.toString() };
    }
    ...
    try {
      ...
      const result = evaluator(trimmed);
